...
export const usersController = {
  ...
  /** Revoke the connection from someone who is currently following the logged-in user. */
  async revokeConnection(followerId: string): Promise<{ ok: boolean; error?: string }> {
    const res = await fetch("/api/users/revoke-connection", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
